    ...
    class HierarchicalCluster(_HCC, ABC):
        ...
        def find_child(self):
            ...
            for ii,j in enumerate(p):
                # calculate count
                individual_counts.append((len(j) + sum(ing[ii]) + (int(len(intersection(i, j)) == len(i)))))
            ...
            # exact match
            counts = (np.argmax(ic), max(individual_counts), fuzzy_intersection)
...
def cluster_model(MAX_CLUSTERS):
    ...
    def the_model(clusters, MAX_CLUSTERS, env=None):
        ...
        while True:
            ...
            if node.terminal:
                print(mcts.print_nodes(df, df_export=True))
                break
